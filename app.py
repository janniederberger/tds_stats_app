...
# -----------------------------
    ...
    # --- Sortierte DataFrames für Charts ---
    ...
    # --- Farb-Setup für Kacheln ---
    card_bg_color = "#f0f0f0"  # Hintergrundfarbe Kachel
    ...
    # -----------------------------
    # Winrate Charts Section (Total, Adventurer, Guardian)
    # -----------------------------
    charts_content = ""
    ...
    st.markdown(card("<h3>Total Ranking</h3>"), unsafe_allow_html=True)
    st.plotly_chart(fig_total, use_container_width=True, key="total_ranking_chart")
    ...
    st.markdown(card("<h3>Adventurer Ranking</h3>"), unsafe_allow_html=True)
    ...
    st.markdown(card("<h3>Guardian Ranking</h3>"), unsafe_allow_html=True)
    st.plotly_chart(fig_guard, use_container_width=True, key="guard_ranking_chart")

    # -----------------------------
    # Single Player Analysis (große Kachel, 3 kleine Pie-Charts)
    # -----------------------------
    player_sel = st.selectbox("Select Player", df_stats_sorted["player"].tolist())
    player_row = df_stats[df_stats["player"]==player_sel].iloc[0]

    single_content = f"<h3>Single Player Analysis: {player_sel}</h3>"
    with st.container():
        st.markdown(card(single_content), unsafe_allow_html=True)
        cols = st.columns(3)
        # Total
        with cols[0]:
            st.markdown(f"**Total Winrate**: {int(player_row['winrate']*player_row['games'])}/{player_row['games']}")
        ...
        # Adventurer
        with cols[1]:
            st.markdown(f"**Adventurer Winrate**: {int(player_row['adv_rate']*player_row['adv_games'])}/{player_row['adv_games']}")
            fig = go.Figure(go.Pie(
                labels=["Wins","Losses"],
        ...
        # Guardian
        with cols[2]:
            st.markdown(f"**Guardian Winrate**: {int(player_row['guardian_rate']*player_row['guard_games'])}/{player_row['guard_games']}")
            fig = go.Figure(go.Pie(
                labels=["Wins","Losses"],
    ...
    # -----------------------------
    # Team Analysis (große Kachel, 3 kleine Pie-Charts)
    # -----------------------------
    ...
    if combo_row:
        with st.container():
            st.markdown(card(f"<h3>Team Analysis: {player1} & {player2}</h3>"), unsafe_allow_html=True)
            cols = st.columns(3)
            # Team Total
            with cols[0]:
                fig = go.Figure(go.Pie(
    ...
    if not df_pairs.empty:
        st.markdown(card("<h3>Top Teams</h3>"), unsafe_allow_html=True)
    ...
    # -----------------------------
    st.markdown(card("<h3>Wins by Number of Players (%)</h3>"), unsafe_allow_html=True)
